...
const BENEFIT_ICONS = [Code2, Zap, Shield, Layers]
...
export function Trust() {
  ...
  const benefits = useMemo(
    () =>
      trust.benefits.map((benefit, index) => ({
        ...benefit,
    ...
    <section className="py-20 bg-secondary border-t border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="text-lg text-foreground/60 max-w-2xl mx-auto">{trust.subtitle}</p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-background p-12 shadow-[inset_0_1px_0_0_rgba(204,52,49,0.12)]">
          <h3 className="mb-8 text-center text-2xl font-bold text-foreground">
            {trust.audiencesTitle}
          </h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {trust.audiences.map((audience, idx) => (
              <div key={audience.title} className="space-y-2 text-center">
                <p
